...
void* mm_heap_end = (void*)0x00200000;
...
void* sbrk(size_t size) {
	...
	if (((unsigned int)mm_heap_end + size) < mm_heap_cap) {
		...
		if (mm_heap_cap * 2 > 0x04000000)
			panic(__FILE__, __LINE__, "sbrk(%d) exceeded mm_heap_cap, mm_heap_cap * 2 > 64 MiB.\n", size);
...
#pragma region C++ operators -- poorly implemented, try to avoid using
